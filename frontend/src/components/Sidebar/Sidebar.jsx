...
const Sidebar = () => {
  ...
  const getPlaylists = async () => {
    ...
    const res = await fetch(`${process.env.BACKEND_URL}/api/playlist/`, {
      method: "GET",
      headers: {
  ...
  return (
    <div className="w-1/4 fixed left-0 mt-2 top-0 sidebar text-black ">
      ...
      <div className="mt-2 secondary_bg rounded-lg px-2 py-2">
        ...
        <div className="btns flex gap-4 mb-4  text-black">
          <Link
            to={"/"}
            className="rounded-full mt-4 px-3   py-1 bg-white text-black text-sm"
          >
            Playlists
          </Link>
          <Link
      ...
      <div className="mt-4 px-4 flex gap-4 flex-wrap text-black">
        <a className="text-xs  mx-4" href="#">
          Legal
        </a>
        <a className="text-xs  mx-4" href="#">
          Privacy Center
      ...
      <button className="mx-4 mt-12 text-sm border-black border rounded-full flex gap-2 px-3 py-1 items-center  text-white ">
        <TbWorld />
